...
for ($a=0; $a<$chpsNb; $a++){ 
	...
	//if ($a==10) {$$chps[$a]=preg_replace('/,/',".",$$chps[$a]);}		// PRIX , => . 
}
...
{	if ( $modif ) 
	{	mysqli_query($link,"UPDATE $tableencours SET $addquery1 WHERE ID='$modif'");
		...
		for ($a=1; $a<=$nbr; $a++){		if ( $_FILES['photo'.$a]['name'] ) { $updatefile[$a]=$modif."-".$a.".png";} else {$updatefile[$a]="";} }
	...
	{	mysqli_query($link,"INSERT INTO $tableencours SET $addquery1");
		...
		for ($a=1; $a<=$nbr; $a++){	if ( $_FILES['photo'.$a] ) { $updatefile[$a]=mysqli_insert_id($link)."-".$a.".png"; } }
...
<div id="contenu" style="width:<?=$largtabl*0.6?>px; float:left" class="normalgris">
		...
		<p><span class="libchamps"><i class='fa fa-pencil-square-o fa-1x'></i> Mini texte intro :</span> <input name="<?=$chps[7]?>" value="<?=$$chps[7]?>" type="text"  size="75" /></p>

		<p><span class="libchamps"><i class='fa fa-pencil-square-o fa-1x'></i> Titre :</span><input name="<?=$chps[1]?>" value="<?=$$chps[1]?>" type="text"  size="75" /></p>
        
        <p><span class="libchamps"><i class='fa fa-pencil-square-o fa-1x'></i> Petit texte :</span> <input name="<?=$chps[2]?>" value="<?=$$chps[2]?>" type="text"  size="75" /></p>
        
        <p><span class="libchamps"><i class='fa fa-link fa-1x'></i> Lien :</span> <input name="<?=$chps[6]?>" value="<?=$$chps[6]?>" type="text"  size="70" /></p>
		
        <p><span class="libchamps"><i class='fa fa-calendar fa-1x'></i> Date de tri :</span><input name="<?=$chps[3]?>" value="<?=($$chps[3])?($$chps[3]):(date("d/m/Y"))?>" type="text" size="30" /></p>
    ...
    <div id="contenu" style="width:<?=$largtabl*0.35?>px; float:right" class="normalgris">
		...
		for ($a=1; $a<=$nbr; $a++){ ?>
        <p><span class="libchamps">Photo <?=$a?> / 800x600</span><input type="file" name="photo<?=$a?>"  value=""> </p>
        <?php
		} 	
		//Afficher les photos
		for ($a=1; $a<=$nbr; $a++){
			if ( $modif && file_exists($chemin.$modif."-".$a.".png") ) {	
				redim_img_url($chemin.$modif."-".$a.".png",$wmax,$hmax);
				print("<a href=?modif=$modif&delphoto=$modif-$a.png> <i class='fa fa-trash-o fa-1x'></i></a> photo $a<br/><br/>");
			}
		} 
		?>
    </div><?php } ?>
...
if ($word)  {$result = mysqli_query($link,"SELECT ID,".$liste1." FROM $tableencours WHERE (".$chps[1]." LIKE '%$word%' OR ".$chps[2]." LIKE '%$word%' OR ".$chps[3]." LIKE '%$word%' OR ".$chps[4]." LIKE '%$word%' OR ID LIKE '%$word%') ".$addQ." ORDER  BY dbu DESC ");}
  ...
  { if ($bgcolor=="#EEEEEE") {$bgcolor="#FFFFFF";} else {$bgcolor="#EEEEEE";}
    ...
    print("<td>$ID - $lg</td><td>".$page." <i class='fa fa-angle-right fa-1x'></i></td><td>".strip_tags($titre)."</td><td>".substr(strip_tags($texte),0,60)."...</td><td>".date_barre($dbu)."</td><td><a href=\"?del=$ID&word=$word\"><i class='fa fa-trash-o fa-1x'></i></a></td>");
    print("</tr>");
  }
 ?>
</p>
