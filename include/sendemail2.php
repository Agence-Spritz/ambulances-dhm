...
// VARIABLES
		...
		$ville = isset( $_POST['template-contactform-subject'] ) ? $_POST['template-contactform-subject'] : '';
		$ref = isset( $_POST['template-contactform-ref'] ) ? $_POST['template-contactform-ref'] : '';
		$exp = isset( $_POST['template-contactform-exp'] ) ? $_POST['template-contactform-exp'] : '';
		//$service = isset( $_POST['template-contactform-service'] ) ? $_POST['template-contactform-service'] : '';
		$message = isset( $_POST['template-contactform-message'] ) ? $_POST['template-contactform-message'] : '';
		$subject = isset($subject) ? $subject : 'Recrutement Mss Mouscron';
...
if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
	if( $_POST['template-contactform-email'] != '' ) {
		...
		if( $botcheck == '' ) {
			...
			$mail->Subject = "Job Mss site: ". utf8_decode($subject);

			$name = isset($name) ? "Nom: $name<br>" : '';
			$email = isset($email) ? "Email: $email<br>" : '';
			$phone = isset($phone) ? "Tel: $phone<br>" : '';
			$ville = isset($ville) ? "Ville: $ville<br><br>" : '';
			$ref = isset($ref) ? "References: <br>$ref<br>" : '';
			$exp = isset($exp) ? "Experiences: <br>$exp<br>" : '';
			$message = isset($message) ? "Message: <br>$message<br>" : '';

			$referrer = $_SERVER['HTTP_REFERER'] ? '<br><br><br>Envoy&eacute; depuis: ' . $_SERVER['HTTP_REFERER'] : '';

			$body = utf8_decode("$name $email $phone $ville $ref $exp $message $referrer");
