...
$page = "prod"; 										// Variable pour definir la sous cat page

// PHOTOS
$photosize = "800x500";									// Dimensions idéales d'information pour la photo
...
$redim_w=800; $redim_h=500;
$masquervignette=0;
$nbr=3; // Nombre de photos

// CHAMPS
$chps=array('page','titre','texte','dbu','masquer','lg','rub','ville');
$chpsNb = count($chps);
?>
...
for ($a=0; $a<$chpsNb; $a++){ 
	...
	//if ($a==7) {$$chps[$a]=preg_replace('/,/',".",$$chps[$a]);}		// PRIX , => . 
}
...
{	if ( $modif ) 
	...
	unset($ID,$titre,$texte,$dbu,$masquer,$lg,$rub,$prix);
...
{	$result = mysqli_fetch_array( mysqli_query($link, " SELECT ID,$liste1 FROM $tableencours WHERE ID=$modif ") );
	list($ID,$page,$titre,$texte,$dbu,$masquer,$lg,$rub,$ville ) = $result;
...
<div id="contenu" style="width:<?=$largtabl*0.6?>px; float:left" class="normalgris">
		...
		<p><span class="libchamps"><i class='fa fa-plus-square fa-1x'></i> 1&deg; Rubriques :</span> 
          <select name="<?=$chps[6]?>" />
          
             <?
            $result = mysqli_query($link, "SELECT rub FROM $tableencours WHERE page='prod' GROUP BY rub ORDER BY rub") ;
        ...
        <p><span class="libchamps"><i class='fa fa-money fa-1x'></i> Prix :</span><input name="<?=$chps[7]?>" value="<?=$$chps[7]?>" type="text" required size="25" />&euro;</p>
